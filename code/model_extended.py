...
class Model:
    ...
    def I_eff(self, I, IBn, IBv):
        return I + self.sigma*(IBn+IBv) + self.influx
    ...
    def fun(self, t, y):
        (S,V,Wn,E,EBn,I,IBn,ICU,R,UC,WC,D,C,Wv,ICUv,Rv,EBv,IBv) = y

        dS = -self.gamma*self.Rt(t)*S/self.M*self.I_eff(I,IBn,IBv) - self.Phi(t,UC)
        dV = -(1-self.eta)*self.gamma*self.Rt(t)*V/self.M*self.I_eff(I,IBn,IBv) + (self.Phi(t,UC)+self.phi(t,WC)) - self.omega_v(t,I,IBn, IBv)*V
        dWn =  - self.gamma*self.Rt(t)*Wn/self.M*self.I_eff(I,IBn, IBv)  + self.omega_n(t,I,IBn, IBv)*R - self.Phi(t,UC)
        dWv = - self.gamma*self.Rt(t)*Wv/self.M*self.I_eff(I,IBn, IBv) + self.omega_v(t,I,IBn, IBv)*V + self.omega_n(t,I,IBn, IBv)*Rv - self.phi(t,WC)
        dE = self.gamma*self.Rt(t)*(S)/self.M*self.I_eff(I,IBn, IBv) - self.rho*E
        dEBn = self.gamma*self.Rt(t)*(Wn)/self.M*self.I_eff(I,IBn, IBv) - self.rho*EBn
        dEBv = self.gamma*self.Rt(t)*((1-self.eta)*V+Wv)/self.M*self.I_eff(I,IBn, IBv) - self.rho*EBv
        dI = self.rho*E - (self.gamma+self.delta+self.Theta)*I
        dIBn = self.rho*EBn - (self.gamma + (self.Theta+self.delta)*(1-self.kappa))*IBn
        dIBv = self.rho*EBv - (self.gamma + (self.Theta+self.delta)*(1-self.kappa))*IBv
        dICU = self.delta*(I+ (1-self.kappa)*IBn) - (self.Theta_ICU+self.gamma_ICU)*ICU
        dICUv = self.delta*(1-self.kappa)*IBv - (self.Theta_ICU+self.gamma_ICU)*ICUv
        dR = self.gamma*(I+IBn) - self.omega_n(t,I,IBn, IBv)*R + self.gamma_ICU*ICU
        dRv = self.gamma*IBv - self.omega_n(t,I,IBn, IBv)*Rv + self.gamma_ICU*ICUv
        dUC = self.M*self.Phi(t,UC)
        dWC = self.M*self.phi(t,WC)
        dD = self.Theta*I+(1-self.kappa)*self.Theta*(IBn+IBv)+self.Theta_ICU*(ICU+ICUv)
        dC = (S+Wn+Wv+(1-self.eta)*V)*self.gamma*self.Rt(t)*self.I_eff(I,IBn, IBv)/self.M
    
        return [dS,dV,dWn,dE,dEBn,dI,dIBn,dICU,dR,dUC,dWC,dD,dC,dWv,dICUv,dRv, dEBv, dIBv]
